...
async function renderCalendar() {
    ...
    // Tải cấu hình Ca làm việc để lấy workCount
    const shiftsSnap = await get(ref(db, `COMPANIES/${currentUser.cid}/shifts`));
    const allShifts = shiftsSnap.exists() ? shiftsSnap.val() : {};

    // Lọc các lượt RA (KẾT THÚC) của tháng này
    const monthLogs = attendHistory.filter(l => {
        const d = parseVNToDate(l.date);
        return d.getMonth() === month && d.getFullYear() === year && l.type.includes("KẾT THÚC");
    });

    // Tính tổng công dựa trên workCount của từng ca
    let totalWorkCount = 0;
    monthLogs.forEach(log => {
        const shiftCfg = allShifts[log.shiftCode];
        // Nếu ca có workCount thì cộng theo cấu hình, không thì mặc định là 1 công
        totalWorkCount += shiftCfg ? parseFloat(shiftCfg.workCount || 0) : 1;
    });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Tính tổng công của riêng ngày này
        let dayWork = 0;
        dayLogs.forEach(l => {
            const s = allShifts[l.shiftCode];
            dayWork += s ? parseFloat(s.workCount || 0) : 1;
        });
...
window.changeTab = (id, el) => {
    ...
    if (el.classList.contains('nav-main-icon')) el.classList.add('active'); else el.classList.add('text-blue-500');
};
